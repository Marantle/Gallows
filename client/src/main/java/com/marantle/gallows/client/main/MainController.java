...
public class MainController {
	...
	public void setAddress(String address) {
		...
		logger.info(String.format("Input Address was [%s], address is now [%s]", address,
				this.address));
	}
			...
			public ListCell<Player> call(ListView<Player> param) {
				ListCell<Player> cell = new ListCell<Player>() {
					...
					protected void updateItem(Player item, boolean empty) {
						...
						if (item != null) {
							setText(String.format("%s[%d]", item.getPlayerName(),
...
//        buttonConnect.setDisable(true);
		...
		client.addListener(new Listener() {
			public void received(Connection connection, Object object) {
				if (object instanceof GallowsResponse) {
					...
					logger.info("server responded: [{}]", response.toString());
	...
	void sendRequest(ActionEvent actionEvent) {
		if (!Strings.isNulgitlOrEmpty(textFieldMsg.getText())) {
			...
			Optional<PacketType> requestType = Optional
					.of(requestTypeBox.getSelectionModel().getSelectedItem());
			request.setRoom("Lobby");
			request.setPacketType(requestType.orElse(PacketType.CHAT));
